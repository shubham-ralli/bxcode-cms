...
<script>
    ...
    function loadPickerMedia(reset = false, preSelectId = null) {
        ...
        fetch(url, {
            ...
            .then(data => {
                ...
                if (preSelectId) {
                    // Start a small timeout (300ms) to ensure DOM is fully ready and painted
                    setTimeout(() => {
                        const newItem = document.querySelector(`.media-item[data-id="${preSelectId}"]`);
                        if (newItem) {
                            const clickArea = newItem.querySelector('.cursor-pointer');
                            ...
                            selectMediaItem(preSelectId, url, innerElement);
                        }
                    }, 300);
                }
            })
    ...
    function selectMediaItem(id, url, element) {
        // Remove active class from all
        document.querySelectorAll('#pickerGrid .ring-4').forEach(el => el.classList.remove('ring-4', 'ring-indigo-500'));
        ...
        // If url is empty (e.g. not an image), construct it or get from data
        ...
        if (item) {
            ...
            // Re-construct absolute URL if needed, or use the one passed from DOM
            if (!url || url === '') {
                // Try to construct standard storage URL
            ...
            // Use Intl.DateTimeFormat for better formatting "24/01/2026"
