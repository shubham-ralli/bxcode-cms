            ...
            <tr class="hover:bg-gray-50 transition-colors group">
                ...
                <td class="px-5 py-4">
                    <div class="text-sm font-medium text-gray-900">
                        ...
                        @if($post->status === 'draft')
                            <span class="text-xs text-orange-600 font-bold ml-1">— Draft</span>
                        @endif
                        @if($post->status === 'private')
                            <span class="text-xs text-gray-500 font-bold ml-1">— Private</span>
                        @endif
                        @php
                            $isScheduled = $post->published_at && $post->published_at->isFuture() && in_array($post->status, ['publish', 'scheduled']);
                        @endphp
                        @if($isScheduled)
                            <span
                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 ml-1">
                ...
                <td class="px-5 py-4 text-sm text-gray-500">
                    @if($post->published_at)
                        {{ $post->published_at->format('M j, Y') }}<br>
                        <span class="text-xs">
